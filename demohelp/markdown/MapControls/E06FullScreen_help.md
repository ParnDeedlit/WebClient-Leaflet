...
<body>
    ...
    <div class="zhengwen">
        ...
        &nbsp; &nbsp;3)创建ol.control.FullScreen()类，创建全屏显示控件；<br /><br />
        &nbsp; &nbsp;4)创建ol.map()类，通过设置该类的layers属性加载一张OSM世界地图，再通过设置该类的view属性确定地图的显示中心和级别。<br /><br />
        <br />
        [添加控件]<br />
        <br />
        （4）最后通过设置ol.Map()类的controls属性给地图容器加载全屏显示控件。
        <br />
    </div>
    <div class="xiaobiaoti">
        三、功能类/参数说明
    ...
    <div class="zhengwen">
        ...
        <font class="Class">6、ol.Collection类：</font>地图容器类，用于扩展JS数组，提供快捷操作。<br />
        <br />
        ...
        <font class="Class">7、ol.control.FullScreen类：</font>全屏显示控件，该控件提供一个全屏按钮，当点击的时候，地图充满整个屏幕（注意是整个屏幕，而不是整个浏览器窗口）；当在全屏模式时，在屏幕的右上角会出现一个退出按钮，用于退出全屏模式，同时按 Esc 按钮也可以退出全屏模式。
        <br />
        <br />
        <font class="constructorTile">7.1 构造函数：</font>
        <br />
        <div class="zhuji">
            <font class="constructor">ol.control.FullScreen(opt_options)</font><br />
            <br />
            <font class="paraDescription">参数说明：</font>
            <br />
            <font class="parameter">opt_options：</font>（Object类型）可选项，设置该对象其他属性，以键值对的形式设置。<br />
            <font class="nextParameter">【1】className：</font>（string | undefined）可选项，设置全屏显示控件样式类名。默认为ol-full-screen。<br />
        </div>
        <p>
